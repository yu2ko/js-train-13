...
function searchParamsURL(url) {
  ...
  for (const param of URLSearchParams) {
    vocab.set(param[0], param[1]);
  }
  // URLSearchParams.forEach((value, key) => vocab.set(key, value));
  return vocab;
}
...
function checkURLParameters(url, params) {
  ...
  for (const item of params) {
    newObj[item] = urlObj.searchParams.has([item]);
  }
  return newObj;
}
...
function manipulateQuery(url, options) {
  ...
  if (options.has("append")) {
    const appendGet = options.get("append");
    for (const [key, value] of appendGet) {
      urlObj.searchParams.append(key, value);
    }
  }
  if (options.has("delete")) {
    const deleteGet = options.get("delete");
    for (const key of deleteGet) {
      urlObj.searchParams.delete(key);
    }
  }
  return urlObj.toString();
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  const urlObj = new URL(url);
  const newArray = Array.from(urlObj.searchParams.entries());
  newArray.sort();
  urlObj.search = "";
...
function getURLValues(url) {
  ...
  const keysArr = URLSearchParams.keys();
  const valuesArr = [];
  for (const key of keysArr) {
    const keyValues = URLSearchParams.getAll(key);
    valuesArr.push(...keyValues);
  }
  return valuesArr;
}
